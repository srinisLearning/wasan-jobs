...
export const getApplicationsByJobSeekerId = async (jobSeekerId: number) => {
  try {
    const applicationsResponse = await supabaseConfig
      .from("jobs_application")
      .select("* , job:jobs_jobTable(*) , job_seeker:jobs_users!jobs_application_job_seeker_id_fkey(name , id , email)")
...
export const getApplicationsByJobId = async (jobId: number) => {
  try {
    const applicationsResponse = await supabaseConfig
      .from("jobs_application")
      .select(
        "* , job:jobs_jobTable(*) , job_seeker:jobs_users!jobs_application_job_seeker_id_fkey(name , id , email)",
      )
      .eq("job_id", jobId)
      .order("created_at", { ascending: false });
    if (applicationsResponse.error) {
